...
private:
  ...
  class PairComp {
    private:
      Key_compare compareLess;
    public:
      bool operator()(Pair_type pair1, Pair_type pair2) const {
          return compareLess(pair1.first, pair2.first);
      }
  };
...
public:
  ...
  // HINT: http://www.cplusplus.com/reference/map/map/operator[]/
    Value_type& operator[](const Key_type& k) {
        Iterator kSpot1 = find(k);
        if (kSpot1 == end()) {
            Pair_type dummyPair = {k, Value_type()};
            insert(dummyPair);
        }
        Iterator kSpot2 = find(k);
        return (*kSpot2).second;
    }
  ...
  //           the value true.
    std::pair<Iterator, bool> insert(const Pair_type &val) {
        std::pair<Iterator, bool> pairToReturn;
        if (find(val.first) == end()) {
            pairToReturn.first = theTree.insert(val);
            pairToReturn.second = true;
            return pairToReturn;
        }
        pairToReturn.first = find(val.first);
        pairToReturn.second = false;
        return pairToReturn;
    }
...
private:
    BinarySearchTree<Pair_type, PairComp> theTree;
};
