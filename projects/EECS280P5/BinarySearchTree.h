...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
      ...
      else if (node->right && less(min_element_impl(node->right)->datum, node->datum)) {
          return false;
      }
      else if (node->left && less(node->datum, max_element_impl(node->left)->datum)) {
          return false;
      }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
      if (!node) {
          ...
          Node *newLowest = min_greater_than_impl(node->left, val, less);
          if (newLowest) {
              return newLowest;
          }
          else {
